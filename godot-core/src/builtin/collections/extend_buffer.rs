...
/// Asserts at compile time that `N` is greater than zero. The obvious solution `static_assert!(N > 0)` fails to compile:
/// "error[E0401]: can't use generic parameters from outer item"
/// Discussion and workaround: https://users.rust-lang.org/t/how-do-i-static-assert-a-property-of-a-generic-u32-parameter/76307/2
struct AssertPositive<const N: usize>;

impl<const N: usize> AssertPositive<N> {
    const OK: () = assert!(N > 0, "N must be greater than zero");
}
